...
const API = process.env.NEXT_PUBLIC_API_URL ?? 'https://examifyapi-production.up.railway.app'

function AdmissionPayContent() {
  ...
  useEffect(() => { if (applicantId && school) loadOffer() }, [applicantId, school])
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (d.noFee) { window.location.href = `/admissions/pay/${applicantId}/callback?school=${school}&noFee=true`; return }
      window.location.href = d.authorizationUrl
    } catch (e: any) { setError(e.message); setPaying(false) }
  }

  if (loading) return <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f7f7f5',fontFamily:'system-ui' }}><p style={{color:'#6b6b65'}}>Loading offer details...</p></div>
  if (error) return <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f7f7f5',fontFamily:'system-ui',padding:'1rem' }}><div style={{background:'white',borderRadius:'20px',padding:'2.5rem',maxWidth:440,textAlign:'center' as const}}><p style={{fontSize:'2rem',marginBottom:'1rem'}}>❌</p><p style={{fontSize:'1rem',fontWeight:600,color:'#dc2626',marginBottom:'0.5rem'}}>Unable to load offer</p><p style={{fontSize:'0.875rem',color:'#6b6b65'}}>{error}</p></div></div>
  if (!data) return null
  ...
  if (offer.acceptanceFeePaid) return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f7f7f5',fontFamily:'system-ui',padding:'1rem' }}>
      <div style={{ background:'white',borderRadius:'20px',padding:'2.5rem',maxWidth:480,textAlign:'center' as const,border:'2px solid #1a6b4a' }}>
        ...
        <p style={{fontSize:'0.875rem',color:'#6b6b65'}}>{applicant.name} has already accepted the admission offer.</p>
        <p style={{fontSize:'0.78rem',color:'#a0a09a',marginTop:'0.5rem'}}>Application: {applicant.applicationNumber}</p>
      </div>
    </div>
  )
  ...
  if (expired) return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f7f7f5',fontFamily:'system-ui',padding:'1rem' }}>
      <div style={{ background:'white',borderRadius:'20px',padding:'2.5rem',maxWidth:440,textAlign:'center' as const }}>
        ...
        <p style={{fontSize:'0.875rem',color:'#6b6b65'}}>This admission offer has expired. Please contact {schoolInfo.name} for assistance.</p>
      </div>
    </div>
  )
          ...
          {feeRequired ? (
            <>
              <p style={{fontSize:'0.875rem',color:'#3a3a36',lineHeight:1.7,marginBottom:'1.25rem'}}>
                To accept this offer, please pay the acceptance fee of <strong>₦{offer.acceptanceFeeAmount.toLocaleString()}</strong>. Payment is processed securely via Paystack.
              </p>
              <button onClick={handlePay} disabled={paying}
                style={{width:'100%',padding:'1rem',background:'#1a6b4a',color:'white',border:'none',borderRadius:'12px',fontSize:'1rem',fontWeight:700,cursor:paying?'not-allowed':'pointer',opacity:paying?0.6:1}}>
                {paying ? 'Redirecting to payment...' : `💳 Pay ₦${offer.acceptanceFeeAmount.toLocaleString()} & Accept Offer`}
              </button>
            </>
          ) : (
            <>
              <p style={{fontSize:'0.875rem',color:'#3a3a36',lineHeight:1.7,marginBottom:'1.25rem'}}>Click below to accept this offer. No payment required.</p>
              <button onClick={handlePay} disabled={paying}
                style={{width:'100%',padding:'1rem',background:'#1a6b4a',color:'white',border:'none',borderRadius:'12px',fontSize:'1rem',fontWeight:700,cursor:paying?'not-allowed':'pointer',opacity:paying?0.6:1}}>
...
export default function AdmissionPayPage() {
  return (
    <Suspense fallback={<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f7f7f5',fontFamily:'system-ui'}}><p style={{color:'#6b6b65'}}>Loading...</p></div>}>
      <AdmissionPayContent />
    </Suspense>
  )
}

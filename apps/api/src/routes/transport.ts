  ...
  app.post('/transport/routes', { preHandler: [authenticate, requireRole('school_admin')] },
    async (request: any, reply: any) => {
      ...
      if (stops && stops.length > 0) {
        const schoolId = String(request.schoolId)
        const routeId = String(route.id)
        for (let i = 0; i < stops.length; i++) {
          const sn = String(stops[i].name)
          const ept = stops[i].estimatedPickupTime ? String(stops[i].estimatedPickupTime) : null
          const edt = stops[i].estimatedDropoffTime ? String(stops[i].estimatedDropoffTime) : null
          const so = i
          await db`
            INSERT INTO route_stops (school_id, route_id, name, estimated_pickup_time, estimated_dropoff_time, sort_order)
            VALUES (${schoolId}::uuid, ${routeId}::uuid, ${sn}, ${ept}, ${edt}, ${so})
          `
        }
      }

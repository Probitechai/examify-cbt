...
export async function resultRoutes(app: FastifyInstance) {
    ...
    async (request: any, reply: any) => {
      ...
      if (classLevel && classArm && subject) {
        results = await tdb.query`
          SELECT sr.id, sr.student_id, sr.subject, sr.ca_score, sr.exam_score, sr.total_score,
                 sr.grade, sr.remark, sr.teacher_comment, sr.approved_at, sr.approved_by,
          ...
          AND u.class_level = ${classLevel} AND u.class_arm = ${classArm} AND sr.subject = ${subject}
        ...
        results = await tdb.query`
          ...
          FROM student_results sr JOIN users u ON u.id = sr.student_id
          WHERE sr.term_id = ${termId}::uuid AND sr.school_id = ${request.schoolId}::uuid
          AND u.class_level = ${classLevel} AND sr.subject = ${subject}
          ORDER BY u.full_name, sr.subject
        ` as any[]
      } else if (classLevel) {
        results = await tdb.query`
      ...
      if (classArm) {
        students = await tdb.query`
          ...
          LEFT JOIN student_results sr ON sr.student_id = u.id
            AND sr.term_id = ${termId}::uuid AND sr.subject = ${subject}
            AND sr.school_id = ${request.schoolId}::uuid
          WHERE u.school_id = ${request.schoolId}::uuid AND u.role = 'student'
          AND u.is_active = true AND u.class_level = ${classLevel} AND u.class_arm = ${classArm}
        ...
        students = await tdb.query`
          ...
          LEFT JOIN student_results sr ON sr.student_id = u.id
            ...
            AND sr.school_id = ${request.schoolId}::uuid
          WHERE u.school_id = ${request.schoolId}::uuid AND u.role = 'student'
          AND u.is_active = true AND u.class_level = ${classLevel}
          ORDER BY u.full_name ASC
        ` as any[]
      }
    ...
    async (request: any, reply: any) => {
      ...
      const rows = await tdb.query`
        ...
        ON CONFLICT (term_id, student_id, subject) DO UPDATE SET
          ...
          teacher_comment = EXCLUDED.teacher_comment, entered_by = EXCLUDED.entered_by,
  ...
  app.get('/results/broadsheet', { preHandler: [authenticate, requireRole('school_admin', 'teacher')] },
    async (request: any, reply: any) => {
      ...
      for (const r of results) {
        if (!studentMap[r.student_id]) {
          studentMap[r.student_id] = {
            ...
            admissionNo: r.admission_no, classArm: r.class_arm,
            subjects: {}, total: 0, subjectCount: 0, average: 0, position: null,
          }
        }
        studentMap[r.student_id].subjects[r.subject] = {
          caScore: r.ca_score, examScore: r.exam_score,
          total: r.total_score, grade: r.grade, remark: r.remark,
        }
      ...
      return reply.send({
        broadsheet: {
          termInfo: termRows[0] ?? {}, classLevel, classArm: classArm ?? 'All',
          subjects, students,
          config: { caWeight: config.ca_weight, examWeight: config.exam_weight, showPosition: config.show_position }
        }
      })
    })
  ...
  app.get('/results/report-card', { preHandler: [authenticate] },
    async (request: any, reply: any) => {
      ...
      let position = null
      if (config.show_position && studentRows[0]) {

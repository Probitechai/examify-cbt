 ...
 * Creates the first school (your client) with sample users.
...
async function seed() {
  ...
  `
  console.log(`✅ School: ${school.name} (${school.subdomain}.examify.ng) — ID: ${school.id}`)
  ...
  `
  if (admin) console.log(`✅ Admin: ${admin.full_name}  (password: Admin@1234)`)

  // 3. Create two teachers
  ...
  for (const t of [
    ...
    const [teacher] = await db`
      ...
      RETURNING full_name
    `
    if (teacher) console.log(`✅ Teacher: ${teacher.full_name}  (password: Teacher@1234)`)
  }

  // 4. Create sample students (SS2A)
  ...
  for (const s of students) {
    const [student] = await db`
      ...
      RETURNING full_name
    `
    if (student) console.log(`✅ Student: ${student.full_name}`)
  }

  // 5. Create sample questions (English Language, SS2)
  const [teacher1] = await db`SELECT id FROM users WHERE school_id = ${schoolId} AND role = 'teacher' LIMIT 1`
  if (teacher1) {
    const questions = [
      {
        text: 'Which of the following is a conjunction?',
        options: [{ key: 'A', text: 'Quickly' }, { key: 'B', text: 'Because' }, { key: 'C', text: 'Beautiful' }, { key: 'D', text: 'Underneath' }],
        correct: 'B',
      },
      {
        text: 'Choose the sentence with the correct use of the apostrophe.',
        options: [{ key: 'A', text: "The boys' bags are heavy." }, { key: 'B', text: "The boy's bags are heavy." }, { key: 'C', text: "The boys bags are heavy." }, { key: 'D', text: "The boys bag's are heavy." }],
        correct: 'A',
      },
      {
        text: 'Identify the verb in the sentence: "The students wrote their examinations carefully."',
        options: [{ key: 'A', text: 'students' }, { key: 'B', text: 'examinations' }, { key: 'C', text: 'wrote' }, { key: 'D', text: 'carefully' }],
        correct: 'C',
      },
    ]
    ...
    for (const q of questions) {
      await db`
        INSERT INTO questions (school_id, created_by, type, subject, class_level, question_text, options, correct_answer, marks)
        VALUES (${schoolId}, ${teacher1.id}, 'mcq', 'English Language', 'SS2',
                ${q.text}, ${JSON.stringify(q.options)}, ${q.correct}, 1)
  ...
  console.log('\n   Tenant header for dev: X-School-Subdomain: greensprings')
